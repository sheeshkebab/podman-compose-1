...
class Podman:
    ...
    def run(self, podman_args, wait=True, sleep=1):
        ...
        if wait:
            # RJH not sure what this ouput is, I will disable it for now.
            #print(p.wait())
            p.wait()
        if sleep:
            time.sleep(sleep)
...
@cmd_run(podman_compose, 'exec', 'execute commands within a running container')
def compose_exec(compose, args):

    # verify service name is valid
    if not args.service in compose.container_names_by_service:
        print("Invalid service specified.")
        sys.exit(1)

    service_containers = compose.container_names_by_service[args.service]

    # verify target index is valid (default is index == 0)
    target_index = args.index

    if target_index > len(service_containers) or target_index < 0:
        print("Invalid container index specified.")
        sys.exit(1)

    target_container = service_containers[target_index]
    ...
    # "-T" = "disable TTY"; enable TTY by default
    if not args.T:
    ...
    if getattr(args, 'workdir', None):
        podman_args.append("--workdir")
        podman_args.append(args.workdir)

    # container to run
    podman_args.append(target_container)

    # command to run
    podman_args = podman_args + args.cnt_command

    # RJH disabling this output, should be dependant on Debug==True
    #print("{}".format(podman_args))
...
def compose_run(compose, args):
    create_pods(compose, args)
    # RJH disabling this output, should be only when debug=true or something similar 
    #print(args)
...
def compose_exec_parse(parser):
    ...
    parser.add_argument("-T", action='store_true',
        help="Disable pseudo-tty allocation. By default podman-compose exec allocates a TTY.")
    ...
    parser.add_argument("-i", "--index", type=int, default=0,
        help="Index of container if there are multiple instances of a service (default: 0)"),
